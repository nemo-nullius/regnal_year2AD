'''a module for processing files with different codings'''

# Copyright (c) 2017 Nemo
# The program is distributed under the terms of the GNU General Public License, version 3.0.
...
def read_input_file(from_file_path):
    '''
    save file contents to a list. **DEPRECATED**
    '''
    result = []
...
def read_file(from_file_path, coding='utf-8', return_type='list'):
    '''
    ...
    if bomInfo == None: # No bom
        decoded_text = encoded_text.decode(codeInfo)
    else: # with bom
        if codeInfo == 'utf-8':
            bom = codecs.BOM_UTF8
